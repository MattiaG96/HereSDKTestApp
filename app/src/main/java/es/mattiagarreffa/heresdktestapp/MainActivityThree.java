...
public class MainActivityThree extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
         ...
         */
        ActivityCompat.requestPermissions(MainActivityThree.this, new String[]{
        ...
        new Handler().postDelayed(() -> System.out.println(">>>>>>>>>>>>>>>>>>>>> " + restaurants), 10000);
    ...
    private void addMapMarkers(GeoCoordinates geoCoordinates, int image) {
         ...
         */
        MapImage mapImage = MapImageFactory.fromResource(MainActivityThree.this.getResources(), image); //HERE SDK method to get the image from the Drawable folder and create a marker.
    ...
    private void showRouteOnMap(Route route) {
        ...
        List<GeoCoordinates> x = new ArrayList<>(route.getPolyline().subList(0, route.getPolyline().size() - 1));
        System.out.println(x.size());
        for (int i = 0; i < x.size(); i = i + 20) {
            System.out.println(">>>>>>>>>>>>>>>>>>" + x.get(i).latitude + " / " + x.get(i).longitude + " --------------- " + i);
            search_restaurants(x.get(i).latitude, x.get(i).longitude, 1500);
        }
    }
    ...
    private void search_restaurants(double lat1, double lon1, int metres) {
        ...
        searchEngine.search("restaurant", geoCircle, searchOptions, (searchError, list) -> {
            ...
            for (SearchResult searchResult : list) {
                if (!restaurants.contains(searchResult.title)) {
                    restaurants.add(searchResult.title);
                    addMapMarkers(searchResult.coordinates, R.drawable.poitest);
                }
            }
        });
    }
